...
function titleFrom(basename) {
  return basename.replace(/^[0-9]+\./, "").replace(/\.md$/, "").replace(/-/g, ".");
}
...
async function fetchList() {
  try {
    ...
    if (el) el.innerHTML = `<p>Liste konnte nicht geladen werden: ${e.message}</p>`;
    throw e;
  }
}
  ...
  for (const key of keys) {
    ...
    if (data._file) {
      ...
      if (isWeek) {
        ...
        btn.type = "button";
        btn.className = "week";
        btn.dataset.path = `${base}/${key}`;
        btn.append(makeSummaryRow(key, iconFor(key, false)));
        sum.append(btn);
      } else {
        sum.append(makeSummaryRow(key, iconFor(key, false)));
      }
      det.append(sum);
...
async function buildNav(list) {
  ...
  for (const group of groups) {
    ...
    const subset = list.filter(p => p.startsWith(group.prefix + "/")).map(p => p.replace(group.prefix + "/", ""));
...
async function loadFile(path, a) {
  ...
  try {
    ...
    const title = meta.date ? formatDateTitle(meta.date) : (meta.title ? formatDateTitle(meta.title) : "");
    h2.textContent = title || "";
    const div = document.createElement("div");
    div.innerHTML = html;
    if (div.firstElementChild && /^H1$/i.test(div.firstElementChild.tagName)) div.removeChild(div.firstElementChild);
    card.append(h2, div);
    wrap.appendChild(card);
    document.getElementById("content").innerHTML = wrap.innerHTML;
  } catch (e) {
    document.getElementById("content").innerHTML = `<p>Fehler beim Laden: <code>${path}</code> – ${e.message}</p>`;
  }
}
...
async function loadWeek(folderPath){
  ...
  try{
    ...
    const files=list.filter(p=>p.startsWith(folderPath+"/")&&p.endsWith(".md")).sort(byFolderAware);
    const entries=await Promise.all(files.map(async p=>{
      const res=await fetch(enc(`content/${p}`)+`?v=${Date.now()}`,nocache);
      if(!res.ok)throw new Error(`HTTP ${res.status}`);
      const md=await res.text();
      const {meta,body}=parseFrontmatter(md);
      const date=meta.date||meta.title||"";
      const title=formatDateTitle(date||p.split("/").pop().replace(".md",""));
      const html=marked.parse(body);
    ...
    document.getElementById("content").innerHTML=`<p>Fehler beim Laden der Woche: <code>${folderPath}</code> – ${e.message}</p>`;
  }
}
...
async function init() {
  try {
    ...
    document.getElementById("sidebar").addEventListener("click", e => {
      const weekBtn = e.target.closest("button.week");
      if (weekBtn) { e.preventDefault(); loadWeek(weekBtn.dataset.path); return; }
    ...
    if (weeks.length) {
      ...
      const btn = [...document.querySelectorAll("button.week")].find(x => x.dataset.path === latest);
      if (btn) {
        let d = btn.closest("details");
        while (d) { d.setAttribute("open", ""); d = d.parentElement.closest("details"); }
      }
    } else {
